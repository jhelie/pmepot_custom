...
#
package provide pmepot_jean 1.0.4

namespace eval ::PMEPot_jean {

  package require pmepot_core_jean 1.0.0

  namespace export pmepot_jean
  
  #define the default values presented in the PMEPot GUI
  #-----------------------------------------------------
  #Resolution
...
proc pmepot_jean { args } { return [eval ::PMEPot_jean::pmepot_jean $args] }

# This is the main procedure, called when the Run button is clicked
proc ::PMEPot_jean::pmepot_jean { args } {
  variable debug
  ...
  #if none then display the help/usage message
  #-------------------------------------------
  if {$nargs == 0 || $nargs % 2} {
  ...
  #browse the list of arguments by pair in order to extract the value of all the parameters
  #----------------------------------------------------------------------------------------
  ...
  #before running anything check that there are indeed charged atoms within the selection
  #--------------------------------------------------------------------------------------
  ...
  #this calls the pmepot_create C function in pmepot.c which intializes the data structure "data" containing all the information
  #-----------------------------------------------------------------------------------------------------------------------------
  ...
  #This is where we go through all the frames and calculate the potential!
  #*********************************************************************
  set errFlag [catch {
    for { set f $frames_begin } { $f <= $frames_end } { incr f $frames_step } {
      
      #display the current frame nb
      ...
      #call the pmepot_add C function, defined in pmepot.c (and understood thanks to tcl_pmepot.c)
      pmepot_add $pme $cell [$sel get {x y z charge}]
    }
  } errMsg]
  #*********************************************************************
    
  #go back to intial selection if necessary
  #----------------------------------------
  $sel frame $oldselframe
  if { $updatesel } { puts "updating selection"; $sel update }
  $tmpsel delete
  
  #stop here and kill pme_data structure if error caught in the above loop and display according error message
  #-----------------------------------------------------------------------------------------------------------
  if { $errFlag } {
    pmepot_destroy $pme
    error $errMsg
  ...
  #regardless of the dx option, the potential values are written to dx file
  #------------------------------------------------------------------------
  puts "writing potential to $dxfile"
  set errFlag [catch {pmepot_writedx $pme $dxfile} errMsg]
  
  #once the values are written in the dx file the pme_data structure is discarded
  #------------------------------------------------------------------------------
  pmepot_destroy $pme
  if { $errFlag } { error $errMsg }

  #load the potential reprentation according to the user option
  #------------------------------------------------------------
  set errFlag [catch {
    switch -- $loadmol {
      none { }
  ...
  #after successful loading the dx file is discarded unless otherwise specified by the user
  #----------------------------------------------------------------------------------------
...
proc ::PMEPot_jean::make_grid_from_cell { cell resolution {max 0} } {
...
proc ::PMEPot_jean::make_padded_cell { mol pad {usersel ""} } {
...
proc ::PMEPot_jean::cell_sin { deg } {
  return [expr cos(acos(0.)*(($deg-90.0)/90.0))]
}
proc ::PMEPot_jean::cell_cos { deg } {
  return [expr -1. * sin(acos(0.)*(($deg-90.0)/90.0))]
}

proc ::PMEPot_jean::get_cell_from_vmd { mol } {
...
proc ::PMEPot_jean::make_namd_cell { origin vmdcell } {
  foreach {a b c alpha beta gamma} $vmdcell { break }
...
proc ::PMEPot_jean::read_xsc_file { filename } {
...
proc ::PMEPot_jean::good_fft_dim { r {max 0} } {
...
proc ::PMEPot_jean::make_vmd_cell { cell } {
...
proc ::PMEPot_jean::TempFile {prefix} {
    set tmpdir [tempdir]
