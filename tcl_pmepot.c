 ...
 *
 * $Id: tcl_pmepot.c,v 1.4 2005/07/20 15:37:40 johns Exp $
 *
 */
...
int tcl_pmepot_add(ClientData nodata, Tcl_Interp *interp, int objc, Tcl_Obj *const objv[]) {

  //Unlike most of the other routines in here this one actually pre-process its arguments
  //and performs some checks before calling the corresponding pmepot_add function in pmepot.c
  ...
  //NB: not sure where atom_count is evaluated, has to be in one of the routines called below, before the size allocation for atoms.
  ...
  //define the stucture atoms as an array of nb_atoms*4 - in order to contain x,y,z and charge for each atom
...
int Pmepot_Init(Tcl_Interp *interp) {
  ...
  *countptr = 0;
  ...
  Tcl_PkgProvide(interp, "pmepot_core_jean", "1.0.0");
