...
#
package provide pmepot_gui_jean 1.0.1

namespace eval ::PMEPot_jean::GUI_jean {

  package require pmepot_jean 1.0.4

  namespace export pmepot_gui_jean
...
#  variable default_ewald_factor 1.0
  ...
  proc gridupdate { } {
    ...
    variable gridB
    variable gridC
    foreach {gridA gridB gridC} [[namespace parent]::make_grid_from_cell [get_cell] $gridResolution $gridMaxDim] { break }
  }
...
#This is the function called when clicking on the "PME Electrostatics" plugin (the one in .vmdrc), it calls the GUI builder
proc pmepot_gui_jean { args } {
  if { [catch { eval ::PMEPot_jean::GUI_jean::pmepot_gui_jean $args } out] } {
    global errorInfo
    puts "$out\n$errorInfo"
    error $out
  } else {
...
proc ::PMEPot_jean::GUI_jean::vmdsetcell {{errmsg 1}} {
  variable currentMol
...
proc ::PMEPot_jean::GUI_jean::copybtocharge {} {
    variable currentMol
...
proc ::PMEPot_jean::GUI_jean::guesscharge {} {
    variable currentMol
    variable atomselectText
...
proc ::PMEPot_jean::GUI_jean::readcharge {} {
    variable w
    variable currentMol
...
# This is the function that builds the GUI! it's the starting point of the plugin and where the calculations are launches from
#*****************************************************************************************************************************
proc ::PMEPot_jean::GUI_jean::pmepot_gui_jean {} {
  variable w
  ...
  wm title $w "PME Electrostatics Jean"
  ...
  #this defines the "Not Periodic" field and updates its values when modified
  #--------------------------------------------------------------------------
  ...
  #this defines the "Selection" field and updates its values when modified
  #-----------------------------------------------------------------------
  ...
  #this defines the "Enclose" button and the corresponding action when clicked
  #---------------------------------------------------------------------------
  ...
  #this defines the "Cell From" buttons and the corresponding actions when clicked
  #-------------------------------------------------------------------------------
  grid [label $f.fromlabel -text "Cell From: "] \
    -row $row -column 0 -sticky e
  grid [button $f.vmdbutton -text "VMD Info" \
    -command ::PMEPot_jean::GUI_jean::vmdsetcell ] -row $row -column 1 -sticky ew
  ...
  #this defines the "Origin" field and updates its values when modified
  #--------------------------------------------------------------------
  ...
  #this defines the "Length" field and updates its values when modified
  #---------------------------------------------------------------------
  ...
  #this defines the "A/B/C Vector" fields and updates their value when modified
  #----------------------------------------------------------------------------
  ...
  #this defines the "Allow Unaligned Cells" button and updates its value when modified
  #-----------------------------------------------------------------------------------
  ...
  #this defines the "Resolution" field and updates its value when modified
  #-----------------------------------------------------------------------
  ...
  #this defines the "Max size" field and updates its value when modified
  #---------------------------------------------------------------------
  ...
  #this defines the "Grid sizes" fields and updates their value when modified
  #--------------------------------------------------------------------------
  ...
  #this defines the "Ewald Factor" field and updates its value when modified
  #-------------------------------------------------------------------------
  ...
  #this defines the "Avg frames" field and updates its value when modified
  #-----------------------------------------------------------------------
  ...
  #this defines the "Update Selection" button and updates its value when clicked
  #-----------------------------------------------------------------------------  
  ...
  #this defines the "Output File" field and updates its value when modified
  #------------------------------------------------------------------------
  ...
  #this defines the "Load into" buttons and update their value when modified
  #-------------------------------------------------------------------------
  ...
  #this defines the "Create new" buttons and update their value when modified
  #--------------------------------------------------------------------------
  ...
  #This defines happens when the "Run PME" button is clicked
  #*********************************************************************
  grid [button $f.button -text "Run PME" \
    -command [namespace code {
        ...
        if { [catch {
          ...
          #this is where the user defined parameters + the structure/traj info is gathered (concatenated in one variable of type list)
          set args [list -sel $sel -cell [get_cell] -loadmol $loadMol -grid [list $gridA $gridB $gridC] -ewaldfactor $ewaldFactor -frames $frames -updatesel $updatesel]
          
          #this test whether a dx file has been supplied and if so appends it to the list of arguments
          if { ! [string equal "" $dxFile] } { lappend args -dxfile $dxFile }
          
          #this calls the procedure pmepot (line 28) in pmepot.tcl: the [namespace parent] is PMEPot, pmepot_gui.tcl is just defining a sub-namespace
          set repMol [eval [namespace parent]::pmepot_jean $args]
          $sel delete
...
proc ::PMEPot_jean::GUI_jean::set_default_cell { } {
#  catch {
    variable w
    variable currentMol
    variable cellPadding
...
proc ::PMEPot_jean::GUI_jean::fill_mol_menu {name} {
