...
int write_dx_grid(FILE *file, const int *dims, const float *oddd,const float *data, float scale, const char *label) {
  ...
  //called by: write_dx_grid(file,data->dims,data->oddd,data->avg_potential,1./data->avg_count,"PME potential (kT/e, T=300K)");
  ...
  //these correspond to the "header" of the dx file
  fprintf(file, "# %s\n", label);
  ...
  //this corresponds to the browsing of the avg_potential
  ll = 0;
  for (ii = 0; ii < ni; ++ii)
    for (jj = 0; jj < nj; ++jj)
      for (kk = 0; kk < nk; ++kk) {
